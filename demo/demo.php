...
//发送文本内容
\XYWeChat\ResponseInitiative::text($tousername, '文本消息内容');
//发送图片
\XYWeChat\ResponseInitiative::image($tousername, $mediaId);
//发送语音
...
//发送音乐
\XYWeChat\ResponsePassive::music($fromusername, $tousername, '音乐标题', '音乐描述', '音乐链接', '高质量音乐链接，WIFI环境优先使用该链接播放音乐', '缩略图的媒体id，通过上传多媒体文件，得到的id');
//发送图文
//创建图文消息内容
$tuwenList[] = array('title'=>'标题1', 'description'=>'描述1', 'pic_url'=>'图片URL1', 'url'=>'点击跳转URL1');
$tuwenList[] = array('title'=>'标题2', 'description'=>'描述2', 'pic_url'=>'图片URL2', 'url'=>'点击跳转URL2');
...
$itemList = array();
foreach($tuwenList as $tuwen){
    $itemList[] = \XYWeChat\ResponsePassive::newsItem($tuwen['title'], $tuwen['description'], $tuwen['pic_url'], $tuwen['url']);
}
\XYWeChat\ResponsePassive::news($fromusername, $tousername, $itemList);
//将消息转发到多客服
\XYWeChat\ResponsePassive::forwardToCustomService($fromusername, $tousername);
...
//----分组管理----
//创建分组
\XYWeChat\UserManage::createGroup('分组名');
//获取分组列表
\XYWeChat\UserManage::getGroupList();
//查询用户所在分组
\XYWeChat\UserManage::getGroupByOpenId($openId);
//修改分组名
\XYWeChat\UserManage::editGroupName('分组Id', '新的组名');
//移动用户分组
\XYWeChat\UserManage::editUserGroup($openId, '新的分组ID');
//---用户管理----
//获取用户基本信息
\XYWeChat\UserManage::getUserInfo($openId);
//获取关注者列表
\XYWeChat\UserManage::getFansList($next_openId='');
//修改粉丝的备注
\XYWeChat\UserManage::setRemark($openId, '新昵称');
//获取网络状态
\XYWeChat\UserManage::getNetworkState();
...
\XYWeChat\WeChatOAuth::getCode($redirect_uri, $state=1, $scope='snsapi_base');
/**
 ...
 */
$code = $_GET['code'];
\XYWeChat\WeChatOAuth::getAccessTokenAndOpenId($code);

//上传多媒体
\XYWeChat\Media::upload($filename, $type);
//下载多媒体
\XYWeChat\Media::download($mediaId);
...
\XYWeChat\Menu::setMenu($menuList);
//获取菜单
\XYWeChat\Menu::getMenu();
//删除菜单
\XYWeChat\Menu::delMenu();
...
//群发消息
//获取粉丝列表
$fansList = \XYWeChat\UserManage::getFansList();
//上传图片
...
$list[] = array('thumb_media_id'=>$menuId['media_id'] , 'author'=>'作者', 'title'=>'标题', 'content_source_url'=>'www.lanecn.com', 'digest'=>'摘要', 'show_cover_pic'=>'0');
$mediaId = \XYWeChat\AdvancedBroadcast::uploadNews($list);
//给粉丝列表的用户群发图文消息
$result = \XYWeChat\AdvancedBroadcast::sentNewsByOpenId($fansList['data']['openid'], $mediaId);